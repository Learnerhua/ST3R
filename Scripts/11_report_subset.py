...
# Activate the subset content override as early as possible so lib modules
# (collect / render_html) that do `import content` see the cell-type wording.
sys.path.insert(0, os.path.normpath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Report_config", "lib")))
import content_subset  # noqa: F401  -- side-effect: monkey-patches content.RESULT_SECTIONS
...
# ------------------------------------------------------------------ #
SUBSET_KEYS = ("06", "07")
SUBSET_NAME = "3D reconstruction (alignment + TDR)"
...
def _print_run_info(root, prefix, sample_list_path, config_path, out_dir, no_pdf):
    """Print the standard subset run-information banner."""
    print("\n==================== Report Generation (Subset) ====================\n")
    print("Current time   :", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    print("Operating system:", platform.system(), platform.release())
    print("Platform       :", platform.platform())
    print("Working directory:", os.getcwd())
    print("Subset         :", SUBSET_NAME, "(keys:", ", ".join(SUBSET_KEYS), ")")
...
# ------------------------------------------------------------------ #
# Collect: locate upstream files for the subset                       #
# ------------------------------------------------------------------ #
def collect_subset_results(root, prefix, sample_list_path, max_embed_mb, embed_enabled):
    """Locate upstream outputs and keep only sections whose key is in SUBSET_KEYS."""
    sys.path.insert(0, _lib_dir())
    import collect
    sections, slices, warnings = collect.collect(
        root, prefix, sample_list_path, max_embed_mb, embed_enabled)
    sections = [s for s in sections if s.get("key") in SUBSET_KEYS]
    if not sections:
        print("Error: No subset sections found.", file=sys.stderr)
        print(f"  Required step dirs: {', '.join(k + '_*' for k in SUBSET_KEYS)}",
              file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description=f"Generate the analysis report (HTML + PDF) for the {SUBSET_NAME} "
                    "subset. Reads only 06_alignment and 07_tdr outputs; other upstream "
                    "steps are skipped. This script shares the Report_config package with "
                    "11_report.py; no upstream analysis is re-run."
    )

    parser.add_argument(
        "-I", "--input", required=True,
        metavar="INPUT_DIR",
        help="Root directory containing 06_alignment/ and 07_tdr/."
    )
    parser.add_argument(
        "-P", "--prefix", default="",
        metavar="PREFIX",
    ...
    parser.add_argument(
        "-O", "--outpath", default="./11_report_subset",
        metavar="OUTPUT_DIR",
        help="Output directory for the report (default: ./11_report_subset)."
    )
    parser.add_argument(
    ...
    # ------------------------------------------------------------------ #
    # Step 1: Locate upstream files for the subset                         #
    # ------------------------------------------------------------------ #
    print("[STEP 1] Locating upstream result files ...")
    sections, slices, warnings = collect_subset_results(
        root, prefix, sample_list_path, max_embed_mb, embed_enabled)
    n_fig = sum(len(s["figures"]) for s in sections)
    print(f"  -> {len(sections)} subset sections, {n_fig} figures, {len(slices)} slices")
    for w in warnings:
        print(f"  {w}")
    ...
    # ------------------------------------------------------------------ #
    # Step 2.5: SKIPPED in subset mode (no csv / json data files)          #
    # ------------------------------------------------------------------ #
    ...
    print("\n====================================================================")
    print("Report generation complete (subset)!")
    print("Output directory:", out_dir)
    print("    report.html / report.pdf / images/ / viz/ / assets/")
